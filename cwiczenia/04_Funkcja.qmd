...
Podczas wykonywania funkcji mogą wystąpić trzy rodzaje sygnałów:
...
2. Napisz funkcję, która zwraca wszystkie liczby od `1` do `n`, które są podzielne
przez `k`.

3. Napisz funkcję, która obliczy odległość euklidesową pomiędzy dwoma punktami.

4. Napisz funkcję, która zamieni stopnie Celsjusza na Fahrenheita oraz wykona
operację odwrotną. Rodzaj konwersji powinien zostać określony przez użytkownika.

5. Napisz funkcję, która wygeneruje ciąg liczb w określonym zakresie (poprzez 
zdefiniowany początek i koniec) wraz z podanym interwałem (np. co 10). W tym 
celu wykorzystaj odpowiedni rodzaj pętli. Ciąg liczb może być rosnący lub malejący.
