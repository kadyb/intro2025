...
- wynik funkcji -- instrukcja `return()` powoduje zakończenie dziłania funkcji
...
```{r}
result = try(sqrt("tekst"), silent = TRUE)
...
zaróno błędy i ostrzeżenia. Dodatkowo, na końcu funkcji można zdefiniować
...
2. Napisz funkcję, która zamieni stopnie Celsjusza na Farenhaita oraz wykona
...
brakujące wartości (`NA`) średnią arytmetyczną (domyślnie) lub wartością
zdefiniowaną przez użytkownika. Do wykrycia brakujących wartości wykorzystaj
funkcję `is.na()`.
