...
zostanie on zapisany. Oprócz tego, można zdefiniować inne argumenty (np. sepeatator
...
testowanego elementu. Jednak, w przetwarzaniu danych często isotniejsze jest
...
w wektorze. W praktyce może posłużyc to na przykład do znalezienia gatunku irysa,
który ma najmniejszą długość płatku kwiata.
...
Brakujące wartości są ozonaczone w R jako specjalne wartości `NA` (*Not Available*).
...
sprawdza, czy każdy element obiektu ma brakującą wartość `NA`. Zwraca ona wektor
logiczny o tej samej długości, z wartością `TRUE` jeśli wystąpiło `NA`.
...
ich generalizację do formy zestawienia najczęściej z uwzględnieniem katagorii
...
do agregacji, natomiast po prawwej stronie wskazujemy zmienną grupującą. Oprócz
...
np. ID. Wyróżnić można kilka rodzjaów połączeń:
...
Podstawowym narzędziem do łączenia ramek danych na podstawie wspólnego atrubytu
...
służa argumenty `all.x` (lewe połączenie), `all.y` (prawe połączenie), `all`
...
Odpowiedzi zapisz w formie komentarzy.
...
ta wartość?
5. Policz ile jest obserwacji, dla których `Petal.Width` jest większe bądź
równe 2 cm.
6. Do ramki danych dodaj nową kolumnę o nazwie `rzadki`. Przyjmie ona wartość
`TRUE`, jeśli wspólczynnik `Petal.Length` do `Petal.Width` będzie większy od 6
(w przeciwnym razie `FALSE`). W tym celu wykorzystaj funkcję poznaną na
poprzednich zajęciach.
7. Utwórz nową ramkę danych, która będzie posiadała tylko 10 obserwacji
o największej wartości `Sepal.Length` posortowanych malejąco. Do utworzonej
ramki danych dodaj nową kolumnę z ID od 1 do 10.
8. Ze zbioru wybierz gatunek `setosa` i zapisz go na dysku jako plik tekstowy
