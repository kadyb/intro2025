...
Do istniejącego wykresy można dodawać kolejne warstwy punktowe lub liniowe
używając funkcji `points()` oraz `lines()`.
...
lub serie danych. Do jej wygenerowania służy funkcja `legend()`, w której
należy określić położenie, nazwy i kolory grup oraz tytuł.
...
2. Stwórz ponownie powyższy wykres, tylko składający się z dwóch krzywych $Y$.
...
3. Stwórz wykres rozrzutu wykorzystując dane ze zbioru `iris` oraz zmienne
`Sepal.Length` i `Sepal.Width`. Jako kolor uwzględnij gatunek irysów (zmienna
`Species`) oraz dodaj legendę do wykresu.
4. Stwórz wykres słupkowy przedstawiający liczebność gatunków irysów.
5. Na podstawie zmiennych `wt` oraz `mpg` ze zbioru `mtcars` stwórz wykres
rozrzutu, następnie samodzielnie wyznacz linię trendu i dodaj ją do wykresu.
6. Stwórz histogram zmiennej `wt`. Ustaw odpowiednie etykiety osi. Wybierz również
